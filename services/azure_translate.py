...
def map_translation_to_original(lyrics_set, response):
    ''' 
    ...
    for idx, translation_info in enumerate(response):
        ...
        original_lyrics = list(lyrics_set)[idx]
...
def create_translation_map(lyrics_set):
    '''
    ...
    translation_map = map_translation_to_original(lyrics_set, response)
...
if __name__ == "__main__":
    azure_translate_key, azure_translate_location = azure_translate_load_credentials()
    constructed_url, params, headers = construct_translation_request(azure_translate_key, azure_translate_location)
    lyrics_set = {'Dos idiomas a la vez.', '大学院生です。'}
    response = translate_text(constructed_url, params, headers, lyrics_set)
    print_response(response)

    translation_map = map_translation_to_original(lyrics_set, response)

    for line in lyrics_set:
