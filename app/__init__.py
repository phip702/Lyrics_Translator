...
def run_rabbitmq_services():
    ...
    producer_thread.daemon = True  #* Mark as daemon so it doesn't block app termination
    consumer_thread.daemon = True  #* Mark as daemon so it doesn't block app termination
...
def create_app(): 
    ...
    logging.info(f"RABBITMQ: {os.getenv("DATABASE_URL", "sqlite:///db.sqlite3")}")
    ...
    logging.info(f"RABBITMQ: {os.getenv("CLOUDAMQP_URL", "amqp://guest:guest@localhost:5672/")}")
